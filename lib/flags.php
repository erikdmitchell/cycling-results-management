...
function uci_results_flags_css( $hook ) {
    wp_enqueue_style( 'flag-icon-style', UCI_RESULTS_URL . '/lib/css/flag-icon.min.css', array(), '0.8.2' );
}
...
function uci_results_get_country_flag( $country = false, $addon = false ) {
    ...
    switch ( strlen( $country ) ) :
        ...
        case 3:
            $code = '';
            foreach ( $flags_countries_arr as $two_letter => $codes ) :
    ...
    switch ( $addon ) :
        case 'full':
            $html .= '<span class="flag-icon flag-icon-' . $code . '"></span> ' . $flags_countries_arr[ strtoupper( $code ) ][0];
            break;
        default:
...
/**
 * uci_results_all_flags function.
 *
 * @access public
 * @return void
 */
function uci_results_all_flags() {
    global $flags_countries_arr;
    ...
    foreach ( $flags_countries_arr as $two_letter => $codes ) :
        ...
        $flags[ $country ] = '<span class="flag-icon flag-icon-' . strtolower( $codes[1] ) . '"></span>';
    endforeach;
